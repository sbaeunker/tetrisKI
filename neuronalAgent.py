...
class neuronalAgent():
    ...
    def _updateQ(self):
        ...
        index1 = np.flatnonzero(np.logical_or(self.rewards[learnSet]<1.1*min(self.rewards[learnSet]) , self.rewards[learnSet] > 0.9* max(self.rewards[learnSet])))    #   Suche alle Rewards kleiner als konst. Wert; neuster Reward wird nicht beachtet
